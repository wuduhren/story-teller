...
class Database {
    ...
    // connect
    function connect($dsn=null, $database=null, $username=null, $password=null, $presistent=false, $encoding=null) {
        if ($this->pdo) return $this;

        global $_CONFIG;
        $id = $this->id;
        $conf = $_CONFIG[$id];
        $dsn = ($dsn) ? $dsn : (isset($conf['dsn']) ? $conf['dsn'] : 'mysql:host=127.0.0.1');
        $encoding = ($encoding) ? $encoding : (isset($conf['encoding']) ? $conf['encoding'] : 'utf8');

        if(stripos($dsn, 'sqlite')!==false)
            $this->pdo = new PDO($dsn);
        else {
            $database = ($database) ? $database : $conf['database'];
            $username = ($username) ? $username : $conf['username'];
            $password = ($password) ? $password : $conf['password'];
            $presistent = ($presistent) ? $presistent : $conf['presistent'];
            if(substr($dsn, -1)!=';') $dsn .= ';';
